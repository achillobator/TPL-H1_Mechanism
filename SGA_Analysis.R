...
sizeFilePath <- dirname("~/Documents/UW/nemhauser_lab/Reporter_SGA/Analysis_Toronto/SGA_Analysis/3669-1_DA/3669-1_DA_2d/3669-1_colony_size/")
setwd("~/Documents/UW/nemhauser_lab/Reporter_SGA/Analysis_Toronto/SGA_Analysis/3669-1_DA/3669-1_DA_2d/3669-1_colony_size/")
colonySize <- do.call(rbind, lapply(list.files(pattern = ".dat$"), 
                                    read.table, skip = 3, colClasses = c(rep("numeric", 4), 
                                                                         rep("NULL", 1)), fill = TRUE, header = FALSE))
names(colonySize) <- c("Row", "Column", "Size", "Circularity")
colonySize <- within(colonySize, {
        BioRep <- bioRepAnswer
        PlateNum <- rep(c(1:14), each = 1536)})


## Integrate colony size data with ORF map for FLEX collection.
setwd("~/Documents/UW/nemhauser_lab/Reporter_SGA/Analysis_Toronto/SGA_Analysis/3669-1_DA/3669-1_DA_2d/")
ORFmap <- read.csv(file = "DAv2_quadruplicate.csv", stringsAsFactors = F)
...
fileName <- file("~/Documents/UW/nemhauser_lab/Reporter_SGA/Analysis_Toronto/SGA_Analysis/3669-1_DA/3669-1_DA_2d/3669-1_2Day_data.csv")
pathName <- dirname("~/Documents/UW/nemhauser_lab/Reporter_SGA/Analysis_Toronto/SGA_Analysis/3669-1_DA/3669-1_DA_2d/")
...
intensityFBorder <- subset(finalintensityData, gene != "YOR202W") 
...
with(intensityNormord, {
        plot(        meanBrightness,
                     meanlogRatio,
                     xlab = "Brightness",
                     ylab = "Mean log(Rnr3-GFP/tdTomato)",
        ...
        plot(	meanBrightness,
              meanlogRatioNorm,
              xlab = "Brightness",
              ylab = "Mean log(Rnr3-GFP/tdTomato) Normalized",
...
plot1 <- ggplot(data=intensityNormord, aes(as.factor(PlateNum), x=as.factor(PlateNum), y=meanlogRatioNorm,color=as.factor(PlateNum))) + 
...
##
# Calculate Z scores old version
...
# Write File
write.csv(Zscore.per.p.all.dat, file = file.path(pathName, paste("3669-1_DA", "byplate_Z.csv", sep = "")), row.names = F)
...
Zscore.Ratio.GFP.RFP.Size.per.p.all.dat[,8] = as.numeric(Zscore.Ratio.GFP.RFP.Size.per.p.all.dat[,8])
...
plot(Zscore.Ratio.GFP.RFP.Size.per.p.all.dat[,8:11])
pheatmap(Zscore.Ratio.GFP.RFP.Size.per.p.all.dat[,8:11],
         cluster_rows=T, 
         cluster_col=T,
         treeheight_row=,
         treeheight_col=,
...
##clustering
hclust()
data_subset <- subset(Zscore.Ratio.GFP.RFP.Size.per.p.all.dat, select = c("gene", "Zscore","Zscore.GFP","Zscore.RFP","Zscore.Size"))
...
write.csv(data_subset, file = file.path(pathName, paste(coreName, "3669-1_NAA_multi-Zscores.csv", sep = "")), row.names = F)

dat_up = merge(hits_up, data_subset, by.x = "gene", by.y = "gene", all.y = FALSE) 
...
dat_up[,12] = as.numeric(dat_up[,12])
dat_up[,9] = as.numeric(dat_up[,9])
dat_up[,10] = as.numeric(dat_up[,10])
dat_up[,11] = as.numeric(dat_up[,11])
pheatmap(dat_up[,9:12],
         cluster_rows=T, 
         cluster_col=T,
         treeheight_row=,
